...
(function () {
  ...
  /* Sync wishlist visual state + wire click on all existing heart buttons */
  ...
  async function renderWishlistPanel() {
    ...
    if (handles.length === 0) {
      container.innerHTML = `
        <div class="bks-wishlist-empty">
          ...
          <a href="/collections/all" class="bks-btn-ghost">Explore the catalog</a>
        </div>`;
      return;
    }
    ...
    shareBtn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8M16 6l-4-4-4 4M12 2v13"/></svg> Share wishlist`;
    ...
    for (const handle of handles) {
      try {
        ...
        card.innerHTML = `
          ...
          <button class="bks-wishlist-card-remove" data-remove="${handle}">Remove</button>`;
  ...
  function initCamerino() {
    ...
    async function renderCamerinoItems() {
      const handles = wlLoad();
      if (!handles.length) {
        container.innerHTML = '<div style="color:#b8b2a6;font-size:13px;padding:8px 0;">Add products to your wishlist to use the Fitting Room.<br><a href="/collections/all" style="display:inline-block;margin-top:10px;font-size:12px;text-decoration:underline;color:#b8b2a6;">Explore</a></div>';
        return;
      }
      container.innerHTML = '';
      for (const handle of handles) {
        try {
          ...
          btn.dataset.tryonSelect   = 'true';
          btn.dataset.tryonImage    = img;
          btn.dataset.tryonTitle    = prod.title;
  ...
  function injectHeartButtons() {
    ...
    document.querySelectorAll(CARD_SELECTORS.join(',')).forEach(item => {
      ...
      btn.title = wlHas(handle) ? 'Remove from wishlist' : 'Add to wishlist';
  ...
  function showWishlistToast(added) {
    ...
    toast.innerHTML = added
      ? `<svg width="13" height="13" viewBox="0 0 24 24" fill="#e74c3c" stroke="#e74c3c" stroke-width="1.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> Saved to wishlist &nbsp;<a href="/pages/bks-members#wishlist">View →</a>`
